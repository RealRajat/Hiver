...
## Decision: Golden Set Human Review Prep

### Decision
Processed `golden_annotation_ai_draft.csv` to create a clean `golden_annotation_review_queue.csv` explicitly marked with `annotation_source = ai_draft`.

### Reasoning
The golden set serves as the ground-truth benchmark for evaluating the AI agent's intent classifier. Since the initial draft was generated by an AI, it is crucial to clearly separate it from human-approved labels. To maintain the integrity of the evaluation, we explicitly mark the source as `ai_draft` and enforce a genuine human review step before freezing it as `golden_set.csv`.

### Evidence
The Hiver assignment specification mandates *hand-labelled* golden examples.

### Trade-offs
This adds a manual review bottleneck to the development pipeline.

### Consequences
The evaluation set will remain pending (`Phase 4 — Evaluation: Golden Set Review In Progress`) until a human reviewer completes the queue and updates the source to `human_reviewed`.
